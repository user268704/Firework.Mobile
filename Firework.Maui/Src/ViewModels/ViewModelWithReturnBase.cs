...
public partial class ViewModelWithReturnBase<TReturn> : ObservableObject, IQueryAttributable
{
    private static string RecipientIdName = "RecipientId.Name";

    private Guid recipientId;

    /// <summary>
    /// Part of <see cref="IQueryAttributable"/> . This is called when the navigation opens this view with parameters.
    /// </summary>
    public virtual void ApplyQueryAttributes(IDictionary<string, object> query)
    {
        recipientId = (Guid)query[RecipientIdName];
    }
    ...
    protected async Task GoBackAsync(TReturn toReturn, string goTo)
    {
        WeakReferenceMessenger.Default.Send(new ViewModelWithReturnMessage<TReturn>(toReturn, recipientId));
        await Shell.Current.GoToAsync(goTo);
    }
